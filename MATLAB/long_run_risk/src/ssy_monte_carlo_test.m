...
function update_state = compute_update_state(x, c_params)
    ...
    z = rho * z + sqrt(1 - rho^2) * sigma_z * randn
    h_z = rho_hz * h_z + simga_hz * randn
    h_c = rho_hc * h_c + sigma_hc * randn
...
function ssy_stat_mc = ssy_function_factory(ssy)
    ...
    function exp_log_lm = ssy_compute_stat_mc()
        ...
        for i = 0:m
            ...
            for t = 0:n
                y = update_state(x, cparams)
                kappa_sum = kappa_sum + eval_kappa(x, y, c_param)
                x = y
            end
